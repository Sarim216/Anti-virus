...
console.log('2 :', );
  ...
  var runScan = async () => {
    ...
    var scanReqData = {
      ...
      customScanFiles: customScanFiles,
    };
    ...
    var data = await useApiRequest(
        "http://127.0.0.1:5000/initiateScans",
        "POST",
        scanReqData,
        "json"
    );
  ...
  var setQuar = (id, detection) => {
    ...
    useApiRequest("http://127.0.0.1:5000/quarFile", "POST", {
      originalPath: id,
      detectionSpace: detection,
    })
      .then((data) => console.log(data))
      .then(() =>
        setdetectedFiles(detectedFiles.filter((item) => item[0] != id))
      );
  };
  ...
  useEffect(() => {
    ...
    interval = setInterval(() => {
      if (
          Object.keys(scanReport)[indx] ==
          Object.keys(scanReport)[Object.keys(scanReport).length - 1]
        ) {
          console.log('scanReport :', scanReport);
    ...
    ipcRenderer.send("ByteAvenger-get-path", "ByteAvenger_GET_APP_PATH");
    ipcRenderer.once("ByteAvenger-get-path", (event, basePath) => {
      ...
      store.set(
        "Threats Blocked",
        store.get("Threats Blocked") + unsafeItem.length
      );
      store.set(
        "Items Scanned",
        store.get("Items Scanned") + Object.keys(scanReport).length
      );
    });
    return () => {
      clearInterval(interval);
                  ...
                  return (
                    <tr key={fileData[0]}>
                      ...
                      <td>
                        <button
                          id={fileData[0]}
                          className="itemStatusPill"
                          style={{ backgroundColor: "lightgreen" }}
                          onClick={(e) => setQuar(e.target.id, fileData[1])}
                        >
                          Quarantine
                        </button>
                      </td>
