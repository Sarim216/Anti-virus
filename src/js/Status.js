...
function Status() {
  ...
  useEffect(() => {
    ...
    ipcRenderer.once('ByteAvenger-get-path', (event, basePath) => {
      ...
      const darkMode = compStyles.getPropertyValue('--ByteAvengerPrimaryDark');
      if(!store.get("Real Time Protection")){
        ...
        cssVariables.style.setProperty('--ByteAvengerPrimary', '#FF1E00');
        statMsg.innerHTML = "At Risk!";
      }else{
        cssVariables.style.setProperty('--scaleAt0', 1);
        cssVariables.style.setProperty('--scaleAt100', 1.4);
        cssVariables.style.setProperty('--ByteAvengerPrimary', '#00B01D');
        statMsg.innerText = "System is Secure"
      }

      if(store.get("Dark Mode")){
        cssVariables.style.setProperty('--ByteAvengerModeUI', darkMode);
        cssVariables.style.setProperty('--ByteAvengerModeText', darkMode);
        document.body.style.color = "white";
      }else{
        cssVariables.style.setProperty('--ByteAvengerModeUI', "white");
  ...
  return (
    <div className="flex_col">
      <h3 style={{ "textAlign": 'center' }}>Welcome to ByteAvenger Antivirus!</h3>
